...
interface DaylightContourChartProps {
  dayOfYear: number;
  axialTilt?: number;
  width?: number;
  height?: number;
...
export default function DaylightContourChart({
  dayOfYear,
  axialTilt = AXIAL_TILT_DEFAULT,
  width = 650,
  height = 420,
}: DaylightContourChartProps) {
  const svgRef = useRef<SVGSVGElement>(null);
  const rawId = useId();
  const uid = rawId.replace(/:/g, ""); // sanitize for SVG id attributes

  // Pre-compute the daylight grid (recompute when axialTilt changes)
  const gridData = useMemo(() => {
    ...
    for (let iy = 0; iy < ny; iy++) {
      ...
      for (let ix = 0; ix < nx; ix++) {
        values[iy * nx + ix] = daylightHours(lat, ix + 1, axialTilt);
      }
    }
  ...
  useEffect(() => {
    ...
    // Color: warm = long daylight, cool = short daylight
    const color = d3
      .scaleSequential()
      .domain([24, 0])
    ...
    // --- Contours ---
    const thresholds = d3.range(0, 25, 1); // every 1 h for smooth fill
    const contours = d3
      .contours()
      .size([nx, ny])
    ...
    // Defs & clip (use unique IDs per instance)
    const clipId = `contour-clip-${uid}`;
    const gradId = `dl-contour-grad-${uid}`;
    ...
    // Contour lines (every 2 h)
    chart
      .selectAll("path.line")
      .data(
        contours.filter(
          (c) => c.value % 2 === 0 && c.value > 0 && c.value < 24
        )
      )
      .enter()
      .append("path")
      ...
      .attr(
        "stroke",
        (d) =>
          d.value === 12 ? "rgba(0,0,0,0.55)" : "rgba(0,0,0,0.18)"
      )
      .attr("stroke-width", (d) => (d.value === 12 ? 1.5 : 0.6));
    ...
    const labelHours = [4, 8, 12, 16, 20];
    const placed: Array<{ x: number; y: number }> = [];

    labelColumns.forEach((col) => {
      labelHours.forEach((targetH) => {
        for (let iy = 0; iy < ny - 1; iy++) {
          const v1 = values[iy * nx + col];
          const v2 = values[(iy + 1) * nx + col];
          if (
            (v1 >= targetH && v2 < targetH) ||
            (v1 < targetH && v2 >= targetH)
          ) {
            const t = (targetH - v1) / (v2 - v1);
            const sx = xGrid(col);
            const sy = yGrid(iy + t);
    ...
    const legendScale = d3.scaleLinear().domain([24, 0]).range([0, lh]);
    lg.append("g")
    ...
    g.append("rect")
      ...
      .on("mousemove", function (event: MouseEvent) {
        ...
        // Format hours
        const hoursStr = `昼間 ${hours.toFixed(1)} 時間`;

        // Update tooltip text
        tooltipLine1.text(dateStr);
        tooltipLine2.text(`緯度 ${latStr}`);
        tooltipLine3.text(hoursStr);

        // Measure text widths for tooltip sizing
